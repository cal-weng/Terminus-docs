...
- The [OlaresDID](https://github.com/beclab/olaresdid-contract-system/blob/main/src/core/OlaresDID.sol) contract plays a crucial role, and it serves as the [DID Registry](./concepts.md#did-registry). View [the contract](https://optimistic.etherscan.io/address/0x5da4fa8e567d86e52ef8da860de1be8f54cae97d).
- Third-party protocols can extend the reputation based on [OlaresDID](https://github.com/beclab/olaresdid-contract-system/blob/main/src/core/OlaresDID.sol). Currently, the following reputation protocols are in place:
  ...
  - [Application Reputation](https://github.com/beclab/olaresdid-contract-system/blob/main/src/taggers/OlaresAppMarketReputation.sol). View [the contract](https://optimistic.etherscan.io/address/0x08065353D266121938B93D4B1071Bb52CD0C0EE4).

## OlaresDID

The OlaresDID contract manages a hierarchical structure derived from [Domain](./olares-id.md#domain) 

![alt text](/images/overview/snowinning/smart-contract-tree.jpg)

### Node

Each node possesses several default attributes. Some Domain Names can be transformed into Olares IDs.

| Attribute         | Description                                                                                                                                                                                                                                                                                           |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| name           | [Domain Name](./olares-id.md#domainname) certain Domain Name can interchange with Olares ID                                                                                                                                                                                                       |
...
- System<br>
  The owners of abstract nodes such as `root`, `com`, `io` belong to the Olares team.

- Individual<br>
  `olares.com` belongs to [Individual Domain](./olares-id.md#individual), the owner belongs to the Olares team.<br>
  `alice.olares.com` , `bob.olares.com` belong to Individual Olares ID, owners belongs to the users themselves.

- Organization<br>
  `org1.com`,`org.io` belong to [Organization Domain](./olares-id.md#organization), owner belong to the admin of the domain.<br>
  `alice.org1.com`,`bob.org2.io`belongs to Organization Olares ID, owners belongs to the users themselves.

- Entity<br>
  `Application Score` belongs to [Entity Domain](./olares-id.md#entity), and the owner belongs to the applicant of the entity.

Organization admins and users can refer to [Domain Management](contract-manager.md) to manage their own nodes and sub-nodes.

:::info
After the project stabilizes, the ownership will be transferred to the multisig address of the DAO organization by the Olares team.
:::

### Tag

The [Tag](https://github.com/beclab/olaresdid-contract-system/blob/main/src/core/TagRegistry.sol) mechanism allows the [OlaresDID](https://github.com/beclab/olaresdid-contract-system/blob/main/src/core/OlaresDID.sol) contract to extend the metadata stored on nodes.
...
Otmoic lp operates as an account with OlaresName. When signing transaction information, it verifies whether the signer's address exists in AuthAddresses. 
...
Regarding these elements, OlaresDID provides a comprehensive solution. We can use Individual and Entity types of DID to represent the object to be evaluated, use Tags to store extended metadata, and utilize OlaresDID's built-in permission management features to verify identities. In the following two cases, you can learn more details.
...
1. Create the entity`otmoic.reputation` in the OlaresDID contract
...
1. Define on `app.olares.com` the Tag named ratings 的 Tag and set Tagger as Application Reputation contract
2. When there is a new version for an app, we create **`<version>.<appname>.app.olares.com`** on the blockchain. 
3. Users sign the comment/ rating info, and submit to the contract Application Reputation
4. Application Reputation contract saves the rating info into Ratings of `<version>.<appname>.app.olares.com`, publish the comment info in the form of event
